...
Run `rails db:migrate` to actually apply the migration and create this table.  Note that this housekeeping task also stores the migration number itself in the database, and by default it only applies migrations that haven't already been applied.  (Type `rails db:migrate` again to verify that it does nothing the second time.)
...
As a last step before continuing, you can now _seed_ the database (i.e. add initial data) with some movies to make the rest of the chapter more interesting. Copy the code below into `db/seeds.rb` and once you have a sense of what it does, run `rails db:seed` to run it.
